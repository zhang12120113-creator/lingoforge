            ...
            {wrongWords.map(word => (
              <div key={word.name} className="flex items-center justify-between p-3.5 card rounded-button hover:border-primary/20 dark:hover:border-primary/20 transition-colors group">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-sm text-content-tertiary dark:text-gray-400 mt-0.5 truncate">{word.trans?.join('；')}</div>
                </div>
                <button onClick={() => removeWord(word.name)} className="ml-3 text-content-tertiary hover:text-indigo-500 transition-colors p-1.5 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-500/10 shrink-0">
