...
import { getErrorBook, removeFromErrorBook } from '../utils/errorBook.js';

export default function WrongBookModal({ onClose, onWordRemoved }) {
  ...
  const handleRemove = useCallback((wordName) => {
    if (pendingDelete === wordName) {
      removeFromErrorBook(wordName);
      setWrongWords(prev => prev.filter(w => w.name !== wordName));
  ...
  const handlePractice = () => {
    ...
    window.location.href = '/typing/error-book/0';
  };
      ...
      <div className="modal-content max-w-lg max-h-[80vh] flex flex-col p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-content dark:text-white">错题本</h2>
            <span className="text-xs bg-gray-100 dark:bg-white/[0.06] text-content-tertiary dark:text-gray-400 px-2 py-0.5 rounded-full">{wrongWords.length}</span>
          </div>
        ...
        {wrongWords.length === 0 ? (
          ...
          <>
            <div className="overflow-y-auto flex-1 space-y-2 pr-1">
              {wrongWords.map(word => (
                <div key={word.name} className="flex items-center justify-between p-3.5 card rounded-button hover:border-primary/20 dark:hover:border-primary/20 transition-colors group">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <div className="font-semibold text-content dark:text-white text-sm">{word.name}</div>
                      {word.wrongCount > 1 && (
                        <span className="text-[10px] bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400 px-1.5 py-0.5 rounded-full">
                          错 {word.wrongCount} 次
                        </span>
                      )}
                    </div>
                    ...
                    {word.dictName && (
                      <div className="text-xs text-content-tertiary/60 dark:text-gray-500 mt-0.5">{word.dictName}</div>
                    )}
                  </div>
                  <button
                    ...
                    className={`ml-3 shrink-0 flex items-center justify-center min-w-[44px] min-h-[44px] rounded-lg transition-colors
                      ${pendingDelete === word.name
                        ...
                        : 'text-content-tertiary hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-500/10'
                      }`}
                  >
            ...
            <button
              ...
              className="mt-4 w-full py-2.5 bg-primary hover:opacity-90 text-white rounded-button font-medium transition shadow-lg shadow-primary/20"
            >
              开始练习错题
            </button>
          </>
